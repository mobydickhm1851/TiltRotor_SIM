...
from rotorpy_tiltrotor.enhanced_dashboard import build_dashboard
...
def main() -> None:
    ...
    origins = [
        ...
        for item in os.getenv(
            "BOKEH_ALLOW_WS_ORIGIN", default_origins
        ).split(",")
        if item.strip()
    ]
    loop = IOLoop.current()
    ...
    if origins == ["*"]:
        print(
            "WARNING: wildcard WebSocket origin is intended only "
            "for development/testing."
        )
    if (
        os.getenv("DASHBOARD_NO_BROWSER", "0")
        not in {"1", "true", "TRUE"}
