...
def test_far_cs_continuous_turbulence_is_vertical_smooth_and_rms_scaled():
    model = FixedUrbanWindModel(WindScenarioConfig(
        enabled=True,
        mode="continuous_turbulence",
        disturbance_amplitude_mps=1.0,
        start_time_s=0.0,
        random_seed=13,
    ))
    state = {
    ...
    assert 0.45 < np.std(steady) < 1.55
    # The regulation-inspired long turbulence scale must produce a much smoother
    # signal than the short-time-constant OU process used in v0.4.1.
    max_wind_accel = np.max(np.abs(np.diff(steady) / dt))
    assert max_wind_accel < 3.0
    assert np.isclose(FAR_CS_TURBULENCE_SCALE_M, 762.0)
...
def test_default_far_cs_turbulence_does_not_crash_transition():
    sim = _new_simulation()
    ...
    wind = FixedUrbanWindModel(WindScenarioConfig(
        enabled=True,
        mode="continuous_turbulence",
        disturbance_amplitude_mps=1.0,
        start_time_s=35.0,
    ...
    for _ in range(int(115.0 / sim.dt)):
        ...
        if sim.t > 40.0:
            ...
            # Rotation quaternion remains the authoritative state.  The norm
            # bound is a coarse no-divergence check independent of Euler wrap.
    ...
    assert max_abs_angle_deg < 90.0


def test_dashboard_labels_continuous_turbulence_as_rms_sigma():
    doc = Document()
    build_dashboard(doc)
    selector = next(
    ...
    assert "Continuous turbulence (FAR/CS 25.341-style)" in selector.options
    selector.value = "Continuous turbulence (FAR/CS 25.341-style)"

    inputs = list(doc.select({"type": NumericInput}))
    amplitude = next(
        item for item in inputs if item.title == "Turbulence RMS sigma [m/s]"
    )
    assert np.isclose(amplitude.value, 1.0)
